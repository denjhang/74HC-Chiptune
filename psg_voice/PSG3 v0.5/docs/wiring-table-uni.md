...
|-----|------|------|
| 1 (PE) | → GND | 不预置 |
| 5 (CI) | ← **~freq_tc (U23 门1, 与 HC161 的 PE 同一根线)** | freq_tc=H(CI=L) 时走一步 |
| 9 (BD) | → +5V | 二进制 |
...
| 8 (GND) | → GND | |
| 16 (VCC) | → +5V | |
| 9-15 (触发器2) | → GND/+5V (不用) | PRE2/CLR2=H, J2/K2=0, CLK2 悬空 |
...
| Pin | 信号 | 连接 |
|-----|------|------|
| 5 (A1),3 (A2),14 (A3),12 (A4) | ← **reg5.Q0-3 (duty4)** | A = duty4 (被减数) |
| 6 (B1),2 (B2),15 (B3),11 (B4) | ← **~counter (U29.Q0-3 经反相)** | B = ~counter (减数反码) |
| 7 (C0) | → **+5V** | 进位输入=1 (补码加法的+1) |
| 9 (C4) | **→ U28 HC153 (比较输出路径)** | C4=1 表示 duty≥counter → 高 (占空比/阈值调制) |
| 4,1,13,10 (Σ1-4) | 悬空 | 和输出不用 (只看进位) |
| 16 (VCC) | → +5V | |
| 8 (GND) | → GND | |
...
| D0-3 | ← U24.Q1-4 (CD4029 输出) | 有毛刺的波形 |
| Q0-3 | → **U33 HC04 (反相成~counter) → U26 HC283 B1-4** + **U27 HC08** | 干净波形 (283要反码, HC08直接用) |
...
| Pin | 信号 | 连接 |
|-----|------|------|
| 1 (OUT1) | → **U31.REF** | 波形模拟量 (给音量 DAC 当 REF) |
| 4-7 (DB7-DB4) | ← U28.Q0-3 (HC153 选出的波形) | 4-bit 波形 |
| 8-11 (DB3-DB0) | → GND | 低 4 位接地 (4-bit 精度) |
| 12 (CS), 13 (WR) | → GND | 常选通常写入 (透明模式) |
| 14 (VDD) | → +5V | |
| 15 (REF) | → +5V | 满量程基准 |
| 16 (RFB) | → GND | |
| 2 (OUT2), 3 (GND) | → GND | |
...
| 4-7 (DB7-DB4) | ← reg4.Q0-3 (vol4) | 4-bit 音量 |
| 8-11 (DB3-DB0) | → GND | 低 4 位接地 |
| 12 (CS), 13 (WR) | → GND | 透明模式 |
| 14 (VDD) | → +5V | |
| 15 (REF) | ← **U30.OUT1** | 被衰减波形 (级联输入) |
| 16 (RFB) | → GND | |
| 2 (OUT2), 3 (GND) | → GND | |
