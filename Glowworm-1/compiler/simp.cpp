...
Tree simplify(int op, Type ty, Tree l, Tree r) {
	...
	case ADD + _I:
		...
		break;
	case CVI + _I:
		xcvtcnst(_I, l->u.v.i, ty, i, (long long)extend(l->u.v.i, ty)); //修改支持64位,2022.2.24
	...
	case CVU + _I:
		if (l->op == CNST + U) {
			...
			if (needconst || !(l->u.v.u > ty->u.sym->u.limits.max.i))
				return cnsttree(ty, (long long)extend(l->u.v.u, ty)); //修改支持64位,2022.2.24
	...
	case CVP + P:
		...
		break;
	case CVI + U:
		xcvtcnst(_I, l->u.v.i, ty, u, ((unsigned long long)l->u.v.i) & ones(8 * ty->size)); //修改支持64位,2022.2.24
	...
	case CVU + F:
		...
		break;
	case CVF + _I:
		xcvtcnst(F, l->u.v.d, ty, i, (long long)l->u.v.d); //修改支持64位,2022.2.24
	...
	case BAND + U:
		...
		if (r->op == CNST + U && r->u.v.u == 0)
			return tree(RIGHT, ty, root(l), cnsttree(ty, 0ULL)); //修改支持64位,2022.2.24
	...
	case BAND + _I:
		...
		if (r->op == CNST + _I && r->u.v.u == 0)
			return tree(RIGHT, ty, root(l), cnsttree(ty, 0LL)); //修改支持64位,2022.2.24
	...
	case OR + _I:
		op = OR;
		/* 0||r => r, 1||r => 1 */
		ufoldcnst(_I, l->u.v.i ? cnsttree(ty, 1LL) : cond(r));  //修改支持64位,2022.2.24
		//ufoldcnst(_I, l->u.v.i ? cnsttree(ty, 1L) : cond(r));
		break;
	case BCOM + _I:
		ufoldcnst(_I, cnsttree(ty, (long long)extend((~l->u.v.i) & ones(8 * ty->size), ty)));  //修改支持64位,2022.2.24
		//ufoldcnst(_I, cnsttree(ty, (long)extend((~l->u.v.i) & ones(8 * ty->size), ty)));
		idempotent(BCOM + U);
		break;
	case BCOM + U:
		ufoldcnst(U, cnsttree(ty, (unsigned long long)((~l->u.v.u) & ones(8 * ty->size))));  //修改支持64位,2022.2.24
	...
	case LSH + _I:
		...
		if (l->op == CNST + _I && r->op == CNST + _I
			...
			return cnsttree(ty, (long long)(l->u.v.i << r->u.v.i));   //修改支持64位,2022.2.24
	...
	case MOD + _I:
		...
		if (r->op == CNST + _I && r->u.v.i == 1)	/* l%1 => (l,0) */
			return tree(RIGHT, ty, root(l), cnsttree(ty, 0LL));   //修改支持64位,2022.2.25
			//return tree(RIGHT, ty, root(l), cnsttree(ty, 0L));
		break;
	case MOD + U:
