...
```
A[7:0]  ← reg_addr (377 输出, 直连)
A[14:8] ← GND (只用低 256 字节)
DI[7:0] ← reg_data (373 输出, 直连)
WE_n    ← data_wr_pulse_n (低有效, 直连, 无隐藏反相)
OE_n    ← RD_n (CPU 读时低有效, 直连)
CE_n    ← GND (始终选中)
DO[7:0] → ram_do (外部读取)
```
...
SPFM 总线输出 data_wr_pulse_n 低有效，直连 62256 WE_n。
SPFM 总线输出 addr_wr_pulse_n 低有效，直连 377 Enable_bar。
全部直连，无外部反相器。
