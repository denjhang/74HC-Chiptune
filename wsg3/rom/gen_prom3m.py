...
ACC_WE  = 0b0100  # bit[2]=1 (不写 acc)
CP273   = 0b0010  # bit[1]=1 (锁存输出)
CLK174  = 0b0001  # bit[0]=1 (锁存 carry)

# 原 16-bit 微码字 (来自 reference/Namco WSG/Pac-Man技术文档 第 209 行)
#   step 0 (ch0 nibble 0, clear adder):  1101_1110_1111_0111 = 0xDEF7
#   step 1-4 (ch0 nibble 1-4):           1101_1110_1111_1111 = 0xDEFF
#   step 5 (ch0 output):                 1011_1111_1111_1111 = 0xBFFF
#   step 6, B (ch1/ch2 nibble 0, clear): 0xDEF7
#   step 7-9, C-E:                       0xDEFF
...
rom3m_16bit = [
    ...
    0xBFFF,  # step 5: ch0 output (cp273 锁存)
...
# 16-bit word 布局: sub-cycle 0 = nibble[3:0] (最右), sub 3 = nibble[15:12] (最左)
# 即: 动作顺序 1→4 对应 sub 0→3, 对应 nibble 从 LSB 到 MSB
rom = bytearray([0xFF] * ROM_SIZE)
for step in range(16):
    word = rom3m_16bit[step]
    for sub in range(4):
        nibble = (word >> (4 * sub)) & 0xF
        addr = (step << 2) | sub
        rom[addr] = nibble
