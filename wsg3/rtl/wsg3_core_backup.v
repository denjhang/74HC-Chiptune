...
module wsg3_core (
    ...
    // 锁存 acc[1:0] (在 step 4 读取 acc[0]，在 step 5 使用)
    reg [7:0] acc01_latch;
    always @(posedge SPFM_CLK or negedge SPFM_RST_n) begin
        if (!SPFM_RST_n)
            acc01_latch <= 8'b0;
        else if (tdm_step == 4'd4 && sub_cyc == 2'd0)
            acc01_latch <= {acc_dout, 4'b0000};  // 读取 acc[0]
        else if (tdm_step == 4'd3 && sub_cyc == 2'd0)
            acc01_latch[7:4] <= acc_dout;  // 读取 acc[1]
    end

    // 相位 = acc[1][0] + acc[0][3:0] (低 5 位)
    wire [4:0] phase_sel = {acc01_latch[4], acc01_latch[3:0]};
